    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          { label: 'Total Stock Value', value: formatPrice(products.reduce((acc, p) => acc + (p.price * (p.stock || 0)), 0)), icon: TrendingUp, color: 'text-green-400' },
          { label: 'Low Stock Alerts', value: products.filter(p => (p.stock || 0) < 10).length, icon: AlertCircle, color: 'text-red-400' },
        ].map((stat, i) => (
          <motion.div
            key={i}
          ...
          >
            <div className="flex justify-between items-start">
              <div>
                <p className="text-zinc-500 text-xs uppercase tracking-widest font-semibold mb-1">{stat.label}</p>
      ...
      <div className="bg-zinc-900/40 border border-white/5 rounded-2xl backdrop-blur-md overflow-hidden">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/10 bg-white/5">
                <th className="px-6 py-4 text-[10px] uppercase tracking-widest font-bold text-zinc-400">Product</th>
                <th className="px-6 py-4 text-[10px] uppercase tracking-widest font-bold text-zinc-400">Category</th>
                <th className="px-6 py-4 text-[10px] uppercase tracking-widest font-bold text-zinc-400">Price</th>
                <th className="px-6 py-4 text-[10px] uppercase tracking-widest font-bold text-zinc-400">Stock</th>
                <th className="px-6 py-4 text-[10px] uppercase tracking-widest font-bold text-zinc-400">Status</th>
                <th className="px-6 py-4 text-[10px] uppercase tracking-widest font-bold text-zinc-400">Rating</th>
                <th className="px-6 py-4 text-[10px] uppercase tracking-widest font-bold text-zinc-400">Actions</th>
              </tr>
            </thead>
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-sm font-bold text-white group-hover:text-accent transition-colors">{product.name}</p>
                  ...
                  <td className="px-6 py-4">
                    <span className={`text-[10px] font-bold uppercase tracking-tighter px-2 py-1 rounded-full border ${getStatusColor(getStatus(product.stock))}`}>
                      {getStatus(product.stock)}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-1">
                      <span className="text-sm font-bold text-white">{product.rating?.toFixed(1) || '4.5'}</span>
        ...
        {filteredProducts.length === 0 && (
          <div className="py-20 text-center">
            <p className="text-zinc-500 font-serif italic text-lg">No matching products in inventory</p>
          </div>
        )}
        ...
        <div className="p-4 border-t border-white/10 bg-white/5 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button className="px-3 py-1 rounded bg-zinc-800 text-zinc-400 text-[10px] font-bold disabled:opacity-50" disabled>PREV</button>
            <button className="px-3 py-1 rounded bg-zinc-800 text-accent text-[10px] font-bold">NEXT</button>
          </div>
        </div>
      </div>
    </div>
