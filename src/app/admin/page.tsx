...
import { Upload, Plus, Package, Image as ImageIcon, CheckCircle, Loader2, List, ShoppingCart, RefreshCw, ChevronRight, LayoutGrid, TrendingUp, Users } from 'lucide-react';
import { formatPrice } from '@/data/products';
import AppImage from '@/components/ui/AppImage';
import InventorySheet from '@/components/admin/InventorySheet';
import AnalyticsDashboard from '@/components/admin/AnalyticsDashboard';

type Tab = 'add' | 'inventory' | 'orders' | 'sheet' | 'analytics' | 'users' | 'items';
...
export default function AdminPanel() {
  ...
  useEffect(() => {
    if (activeTab === 'inventory' || activeTab === 'analytics' || activeTab === 'sheet') fetchProducts();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setTimeout(() => {
          setSuccess(false);
          setFormData({ name: '', description: '', price: '', category: 'Sarees', stock: '', sizes: 'XS, S, M, L, XL' });
          setPreviewUrl(null);
        }, 3000);
      } else {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-10 border-b border-white/10 pb-6 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <p className="text-zinc-400">Control center for Prinsora luxury inventory and sales.</p>
          </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                  <div className="space-y-6">
                    ...
                    <div>
                      <label className="block text-xs uppercase tracking-widest text-zinc-500 mb-2 font-medium">Product Name</label>
                      <input
                        type="text"
                        required
                        value={formData.name}
                    ...
                    <div>
                      <label className="block text-xs uppercase tracking-widest text-zinc-500 mb-2 font-medium">Description</label>
                      <textarea
                        required
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs uppercase tracking-widest text-zinc-500 mb-2 font-medium">Price (₹)</label>
                        <input
                          type="number"
                          required
                      ...
                      <div>
                        <label className="block text-xs uppercase tracking-widest text-zinc-500 mb-2 font-medium">Initial Stock</label>
                        <input
                          type="number"
                          required
                    ...
                    <div>
                      <label className="block text-xs uppercase tracking-widest text-zinc-500 mb-2 font-medium">Category</label>
                      <select
                    ...
                    <div>
                      <label className="block text-xs tracking-widest uppercase text-zinc-500 mb-2 font-medium">Available Sizes (Comma separated)</label>
                      <input
                        type="text"
                  ...
                  <div className="space-y-6">
                      ...
                      <div className={`border-2 border-dashed rounded-2xl flex flex-col items-center justify-center transition-all duration-300 overflow-hidden ${
                        ...
                        {previewUrl ? (
                          <div className="absolute inset-0 w-full h-full">
                            <AppImage src={previewUrl} alt="Preview" fill className="object-cover" />
                    ...
                    <div className="bg-accent/10 border border-accent/20 rounded-xl p-4 flex gap-3">
                      ...
                      <p className="text-xs text-zinc-400 leading-relaxed">
                        Securely uploaded to <strong className="text-accent">Google Drive</strong>. Metadata synchronized with <strong className="text-accent">Google Sheets</strong>.
                      </p>
                    </div>
                  </div>
                </div>
                <div className="pt-6 border-t border-white/10 flex justify-end">
                  <motion.button
                    ...
                    className={`flex items-center gap-2 px-8 py-4 rounded-xl font-semibold text-primary transition-all shadow-lg ${
                      success ? 'bg-green-500 shadow-green-500/20' : 'bg-accent shadow-accent/20 hover:shadow-accent/40'
                    }`}
                  >
                    {isSubmitting ? (
          ...
          {activeTab === 'inventory' && (
            ...
            >
              <div className="flex justify-between items-center">
                ...
                <button onClick={fetchProducts} className="p-2 hover:bg-white/5 rounded-full transition-colors">
              ...
              {isLoading ? (
                ...
                <div className="grid grid-cols-1 gap-6">
                  {products.map((product) => (
                    <div key={product.id} className="bg-zinc-900/50 border border-white/10 rounded-2xl overflow-hidden backdrop-blur-sm group hover:border-accent/30 transition-all flex flex-col md:flex-row">
                      ...
                      <div className="p-6 flex-grow flex flex-col justify-between">
                        <div>
                            ...
                            <div className="flex items-center gap-3">
                              {product.avatar && (
                                <div className="w-10 h-10 rounded-full overflow-hidden border border-accent/30 flex-shrink-0">
                                  <AppImage src={product.avatar} alt="Avatar" width={40} height={40} className="object-cover" />
                                </div>
                              )}
                          ...
                          <p className="text-zinc-400 text-sm line-clamp-2 mb-4 italic">"{product.description || 'No description provided.'}"</p>
                        </div>
                        <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-white/5">
                          <div className="flex gap-6">
                            ...
                            <div>
                              <p className="text-[10px] uppercase tracking-widest text-zinc-500 mb-1">Stock</p>
                              <p className={`font-bold text-lg ${product.stock < 10 ? 'text-red-400' : 'text-green-400'}`}>{product.stock} pcs</p>
                            </div>
                          </div>
                          <div className="flex gap-2">
                            <button className="px-4 py-2 rounded-lg bg-zinc-800 text-white text-xs font-medium hover:bg-zinc-700 transition-colors">Edit</button>
                            <button 
          ...
          {activeTab === 'sheet' && (
            ...
            >
              <div className="mb-6 flex justify-between items-center">
                ...
                <button onClick={fetchProducts} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-accent/10 text-accent text-xs font-bold border border-accent/20 hover:bg-accent/20 transition-all">
                  <RefreshCw className={`w-3 h-3 ${isLoading ? 'animate-spin' : ''}`} /> Sync with Google Sheets
                </button>
              </div>
              ...
              {isLoading ? (
                ...
                <div className="flex flex-col gap-4">
                  {orders.length === 0 ? (
                    ...
                    orders.map((order) => (
                      <div key={order.id} className="bg-zinc-900/50 border border-white/10 rounded-2xl p-6 backdrop-blur-sm hover:border-white/20 transition-all">
                        <div className="flex flex-wrap justify-between items-start gap-4 mb-6">
                          <div>
                            <div className="flex items-center gap-3 mb-1">
                              <span className="text-lg font-bold text-accent font-mono">{order.id}</span>
                              <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded border ${
                                order.status === 'Pending' ? 'text-yellow-400 border-yellow-400/30 bg-yellow-400/5' : 'text-green-400 border-green-400/30 bg-green-400/5'
                              }`}>
                                {order.status}
                              </span>
                            </div>
                          ...
                          <div className="text-right">
                            <p className="text-xs text-zinc-500 uppercase tracking-widest mb-1">Total Amount</p>
                            <p className="text-2xl font-serif font-bold text-white">{formatPrice(order.total)}</p>
                          </div>
                        </div>
                        
                        <div className="grid md:grid-cols-2 gap-8 pt-6 border-t border-white/5">
                          <div>
                            <p className="text-[10px] uppercase font-bold text-zinc-500 tracking-widest mb-3">Customer Info</p>
                            <p className="text-white font-medium">{order.customer}</p>
                            <p className="text-sm text-zinc-400">{order.email}</p>
                            <p className="text-sm text-zinc-400">{order.phone}</p>
                            <p className="text-sm text-zinc-400 mt-2 line-clamp-2 italic">"{order.address}"</p>
                          </div>
                          <div>
                            <p className="text-[10px] uppercase font-bold text-zinc-500 tracking-widest mb-3">Payment & Fulfillment</p>
          ...
          {activeTab === 'users' && (
            ...
            >
              <div className="p-6 border-b border-white/10 flex justify-between items-center">
                ...
                <button onClick={fetchUsers} className="p-2 hover:bg-white/5 rounded-full transition-colors">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {users.map((u, i) => (
                      <tr key={i} className="text-sm text-zinc-300 hover:bg-white/5 transition-colors">
          ...
          {activeTab === 'items' && (
            ...
            >
              <div className="p-6 border-b border-white/10 flex justify-between items-center">
                ...
                <button onClick={fetchOrderItems} className="p-2 hover:bg-white/5 rounded-full transition-colors">
                  <RefreshCw className={`w-4 h-4 text-accent ${isLoading ? 'animate-spin' : ''}`} />
                </button>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {orderItems.map((item, i) => (
                      <tr key={i} className="text-sm text-zinc-300 hover:bg-white/5 transition-colors">
                        <td className="px-6 py-4 font-mono text-accent font-bold">{item.orderId}</td>
                        ...
                        <td className="px-6 py-4 text-accent font-bold">{formatPrice(item.total)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <div className="mt-12 bg-accent/5 border border-accent/10 rounded-2xl p-6 flex flex-col md:flex-row items-center gap-6">
          ...
          <div className="flex-grow text-center md:text-left">
            ...
            <p className="text-zinc-400 text-sm">
              Your inventory is powered by <strong className="text-accent">Google Sheets</strong>. Any changes you make here are synchronized instantly.
              Images are securely hosted on your <strong className="text-accent">Google Drive</strong> to bypass standard storage limits.
            </p>
          </div>
          <button 
